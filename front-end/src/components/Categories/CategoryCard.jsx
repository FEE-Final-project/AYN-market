...
export default function CategoryCard({ edges, node, handlePrevClick, index, isCustomer }) {
  ...
  return (
    <>
      ...
      <div className=' frontCategoryCard cursor-pointer z-50 bg-gray-300'>
      <p className='my-2 text-center font-bold text-2xl md:text-4xl  text-gray-900 absolute  left-2/4'>{node.categoryName.toUpperCase()}</p>
      <img  src={
            node.image
              ?"http://localhost:8000/" +
                node.image
              : categoryImage
          } className='rounded categoryImage bg-blend-darken mix-blend-multiply '  alt="category logo" />
      </div>
      <div className='backCategoryCard bg-violet-800 rounded  flex flex-col items-center justify-center'>
       ...
       : showEditForm === node.id ?
        ...
        <div key={node.id} className={edges.length < 3 ? "relative  w-full flex flex-col items-center shadow rounded lg:mx-2 my-5" : "relative flex flex-col items-center  shadow rounded lg:mx-2 my-5 lg:w-1/3"} >
          ...
          <img  src={
            node.image
              ?"http://localhost:8000" + node.image
    
              : categoryImage
          } className='w-64' alt="category logo" />
          {/* delete button */}
