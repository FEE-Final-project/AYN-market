...
export default function CategorySlider() {
    ...
    //TODO: fix refetch after the first page bug
      ...
      return (
        <>
          {
            edges.length === 0 ? <div className='bg-red-500 p-5 rounded text-center text-white  my-10 '>no categories added yet </div> : 
          ...
          <button
            ...
            {edges.map(({ node }) => (
              <div key={node.id} className='relative flex flex-col items-center shadow rounded mx-2 my-5 w-1/3'>
                ...
                <button className='absolute left-1 top-1 bg-red-500 rounded p-1' onClick={()=>handleDelete(node.id)}><i className="ri-delete-bin-line text-white"></i></button>
              </div>
            ))}
               <button
